...
// ***** PIN ASSIGNMENT *****
int ssr = 5;
int thermocoupleSO = A3;
int thermocoupleCS = A4;
int thermocoupleCLK = A5;
int lcdRs = 7;
int lcdE = 8;
int lcdD4 = 9;
int lcdD5 = 10;
...
void loop() {
    ...
    if (millis() > nextCheck) {
        ...
        if (reflowState == REFLOW_STATE_ERROR) {								        // If currently in error state
			...
			lcd.setCursor(0, 0);												        // Move the cursor to the 2 line
            lcd.print("Attach Thermocouple!");										            // No thermocouple wire connected
        
		}
        else {
			lcd.setCursor(17, 0);												        // Move the cursor to the 2 line
    ...
    switch (reflowState) {															// Reflow oven controller state machine
        ...
        case REFLOW_STATE_IDLE:
            ...
            if (digitalRead(buttongo) == 0 and digitalRead(Door) == 0) {			// If button is pressed with door closed start reflow process

				while (digitalRead(buttongo) == 0) {
				}
            ...
            if (digitalRead(buttongo) == 0 and digitalRead(Door) == 1) {			// If button is pressed with door open
        ...
        case REFLOW_STATE_SOAK:     
            ...
            if (millis() > timerSoak) {												// If micro soak temperature is achieved   
                ...
                if (setpoint > TEMPERATURE_SOAK_MAX) {
                    ...
                    reflowOvenPID.SetTunings(PID_KP_REFLOW, PID_KI_REFLOW, PID_KD_REFLOW);                    // Set agressive PID parameters for reflow ramp
                    setpoint = TEMPERATURE_REFLOW_MAX;              		    	  // Ramp up to first section of soaking temperature   
                    reflowState = REFLOW_STATE_REFLOW;                   			 // Proceed to reflowing state
    ...
    if (digitalRead(buttongo) == 0) {												// If button is pressed
	...
	if (digitalRead(Door) == 1 and reflowStatus == REFLOW_STATUS_ON) {				//Door opened during a cook?

		lcd.clear();
		lcd.print("Door Open - Reflow Abort!");
